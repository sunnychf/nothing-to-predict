...
def panel_label(ax, label):
    """Bold lowercase panel label with a fixed physical offset from the axes' top-left corner."""
    off = ScaledTranslation(-4 / 72, 3 / 72, ax.figure.dpi_scale_trans)
    ax.text(0, 1, label, transform=ax.transAxes + off, fontsize=8, fontweight="bold", ha="left", va="bottom")


def equalise_row(fig, axes, pad_pt=5.0, right_pad_pt=6.0):
    """Give the axes of one row identical plot-area widths, heights and gutters.
    Constrained layout sizes each column by its own decorations, so an
    equal-span grid ends up with unequal plot areas; the alignment gate requires
    equal ones. Every gutter is set to the widest decoration (tick labels,
    axis label) any non-first panel carries, so nothing overlaps."""
    fig.canvas.draw()
    fig.set_layout_engine("none")
    rend = fig.canvas.get_renderer(); px_per_pt = fig.dpi / 72.0
    W = fig.get_figwidth() * 72.0
    dec = []
    for a in axes:                       # decoration width left of the plot area, in points
        tb, pb = a.get_tightbbox(rend), a.get_window_extent(rend)
        dec.append((pb.x0 - tb.x0) / px_per_pt)
    left = dec[0] + pad_pt; gutter = max(dec[1:]) + pad_pt
    n = len(axes); w = (W - left - right_pad_pt - (n - 1) * gutter) / n
    pos = [a.get_position() for a in axes]
    h0 = min(p.height for p in pos); y0 = max(p.y0 for p in pos)
    for i, a in enumerate(axes):
        a.set_position([(left + i * (w + gutter)) / W, y0, w / W, h0])
...
ax[0].set_xlabel("horizon h"); ax[0].set_ylabel("mean departure / σ"); ax[0].set_title("signed mean departure")
ax[1].set_xlabel("horizon h"); ax[1].set_ylabel("fraction forecast up"); ax[1].set_title("fraction above last value"); ax[1].set_ylim(0.3, 1.0)
ax[2].set_xlabel("departure at h = 128 / σ"); ax[2].set_title("spread at h = 128")
...
for a, lab in zip(ax, "abc"):
    panel_label(a, lab)
_nleg = len(DIR_FILES)
fig.legend(*ax[0].get_legend_handles_labels(), loc="lower center", ncol=5 if _nleg <= 5 else 4, bbox_to_anchor=(0.5, -0.01),
           handlelength=2.2, columnspacing=1.2)
fig.get_layout_engine().set(rect=(0, 0.13 if _nleg <= 5 else 0.24, 1, 0.87 if _nleg <= 5 else 0.76))
equalise_row(fig, ax)
...
for a, t, lab in zip(ax, ("Moirai-1.1, patch size", "TimesFM-2.0, frequency indicator", "FinCast, frequency indicator"), "abc"):
    zero_line(a); a.set_title(t); a.set_xlabel("horizon h"); a.set_xticks([1, 32, 64, 96, 128]); panel_label(a, lab)
...
zero_line(ax[0]); ax[0].set_ylabel("N1 skill vs persistence"); ax[0].set_title("skill on the null"); ax[0].set_ylim(-0.24, 0.09)
ax[1].plot(params, up16, "o-", color=c, ms=3.2, label="h = 16"); ax[1].plot(params, up64, "s--", color=c, ms=3.0, label="h = 64")
zero_line(ax[1], 0.5); ax[1].set_ylabel("N1 fraction forecast up"); ax[1].set_title("upward bias"); ax[1].set_ylim(0.4, 1.0)
ax[2].plot(params, n4, "o-", color=c, ms=3.2); zero_line(ax[2], 100)
ax[2].set_ylabel("N4 skill, % of optimum"); ax[2].set_title("share of the N4 optimum")
for a, lab in zip(ax, "abc"):
    a.set_xscale("log"); a.set_xticks(params); a.set_xticklabels(sizes, rotation=35, ha="right", rotation_mode="anchor")
    a.set_xlabel("Chronos-T5 parameters"); a.minorticks_off(); panel_label(a, lab)
...
zero_line(ax[0]); ax[0].set_ylabel("mean departure / σ"); ax[0].set_title("signed mean departure"); ax[0].legend(loc="upper right")
...
zero_line(ax[1], 0.5); ax[1].set_ylabel("fraction forecast up"); ax[1].set_title("fraction above last value"); ax[1].set_ylim(0.2, 1.0); ax[1].legend(loc="lower left", ncol=2, columnspacing=1.0, handlelength=1.8)
for a, lab in zip(ax, "ab"):
    a.set_xticks(x); a.set_xticklabels(labels); a.set_xlabel("price level of the same increments"); panel_label(a, lab)
...
if REAL and os.path.exists(os.path.join(RES, "real_windows.npz")):
    ...
    for kind, a in (("sur", ax[0]), ("raw", ax[1])):
        ...
        zero_line(a)
    zero_line(ax[2])
    ax[0].set_title("real-increment martingale"); ax[1].set_title("raw series"); ax[2].set_title("raw series, skill")
    ax[0].set_ylabel("mean departure / σ (returns)"); ax[1].set_ylabel("mean departure / σ (returns)"); ax[2].set_ylabel("skill vs persistence")
    for a, lab in zip(ax, "abc"):
        a.set_xlabel("horizon h"); a.set_xticks([1, 32, 64, 96, 128]); panel_label(a, lab)
    ax[2].set_ylim(-0.85, 0.35)
    for j, (name, (lo, hi)) in enumerate(offscale.items()):                 # stacked at the top, where no curve runs
        ax[2].text(2, 0.30 - 0.11 * j, f"{SHORT[name]} below {hi:+.1f}, not drawn", fontsize=6, color=COL[name], va="top")
        facts["real"][name]["raw"]["skill_h16_to_128_range"] = [lo, hi]
    hnd, lab = ax[1].get_legend_handles_labels()
    _nc = 3 if len(lab) <= 6 else 4
    fig.legend(hnd, lab, loc="lower center", ncol=_nc, bbox_to_anchor=(0.5, -0.01), handlelength=2.2, columnspacing=1.4)
    fig.get_layout_engine().set(rect=(0, 0.2 if len(lab) <= 6 else 0.22, 1, 0.8 if len(lab) <= 6 else 0.78))
    equalise_row(fig, ax)
...
if os.path.exists(os.path.join(RES, "remedy_summary.json")):
    ...
    ax[0].set_title("○ raw  ● mirror  ▲ sign-avg."); ax[1].set_title("positive control"); ax[2].set_title("real-increment martingale")
    ax[2].set_ylim(-0.85, 0.35)                                             # Sundial's error bars reach -0.81; nothing drawn is clipped
    for j, (name, vals) in enumerate(off_c):
        ax[2].text(0.04, 0.96 - 0.1 * j, f"{SHORT[name]} below {np.nanmax(vals):+.1f}", transform=ax[2].transAxes, fontsize=6, color=COL[name], va="top")   # top-left: empty above the markers
    for a_, lab in zip(ax, "abc"):                                            # staggered rows so that neighbouring labels never touch
        ...
        a_.set_xticks(xs); a_.set_xticklabels(["\n" * (i % _rows) + SHORT[n] for i, (n, _) in enumerate(NAMES)], fontsize=6.5 if len(NAMES) <= 8 else 6); panel_label(a_, lab)
    equalise_row(fig, ax)
...
json.dump(facts, open(os.path.join(OUT, "figures_facts.json"), "w"), indent=1)
