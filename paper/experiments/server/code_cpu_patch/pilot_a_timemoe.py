...
def load_model():
    m = AutoModelForCausalLM.from_pretrained(MODEL_ID, device_map=__import__("os").environ.get("TSFM_DEVICE","cuda"), trust_remote_code=True); m.eval()
...
def forecast_tm(model, ctx_np, H, batch=64):
    ...
    outs = []; i, bs = 0, batch
    while i < len(ctx_np):
        x = torch.tensor(np.asarray(ctx_np[i:i + bs], dtype=np.float32), device=__import__("os").environ.get("TSFM_DEVICE","cuda"))
