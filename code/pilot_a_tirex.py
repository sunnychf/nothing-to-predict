"""Pilot A driver for TiRex (NX-AI, 2025): an xLSTM-based zero-shot forecaster, `NX-AI/TiRex`
via the tirex-ts package in the `tirex` env (sLSTM kernels off; the package's PyTorch fallback).

Shares generators and metrics with the other drivers through nulls.py / probe_metrics.py;
same JSONL schema and key format.

Particulars (smoke_tirex.py, 2026-09-17): 35.3M parameters; forecast(context, prediction_length)
returns nine quantiles (0.1 .. 0.9) and a second output the package calls the mean, which equals
the 0.5 quantile exactly (max |diff| = 0), so the point forecast used here is the median, the
model's own designated summary, as for Chronos-Bolt and Chronos-2 (the average of the nine
quantiles differs from it by up to 0.56 sigma on a random walk and is not a mean either).
Deterministic, no sampling, no Monte Carlo term; continuous scaling, floor = norm_floor.
Horizon-prefix-equivalent (H=16 vs the first 16 of H=128: rms 0). Accepts contexts of 512 to
4096 points. Takes values only: no frequency, timestamp or calendar input, so the
declared-frequency test is undefined for it, as for the Chronos family and Time-MoE.

Usage (inside env_tirex.sh):  python pilot_a_tirex.py --stage all
"""
...
MODEL_ID = os.environ.get("TIREX_MODEL", "NX-AI/TiRex"); MODEL_TAG = "tirex"
CTX = 512
...
@torch.no_grad()
def forecast_tirex(model, ctx_np, H, batch=64):
    """(mean (n,H), quantiles (n,H,9)); chunked, OOM-adaptive."""
    pts, qs = [], []; i, bs = 0, batch
    while i < len(ctx_np):
        x = torch.tensor(np.asarray(ctx_np[i:i + bs], dtype=np.float32))
        try:
            q, m = model.forecast(context=x, prediction_length=H)
        except torch.OutOfMemoryError:
            torch.cuda.empty_cache()
            if bs == 1: raise
            bs = max(1, bs // 2); log(f"  OOM -> batch {bs}"); continue
        q, m = _np(q), _np(m)
        if q.ndim == 3 and q.shape[1] == 9 and q.shape[-1] != 9:
            q = np.transpose(q, (0, 2, 1))                       # (n, 9, H) -> (n, H, 9)
        q, m = q[:, :H, :], m[:, :H]
        if not _checked[0]:
            log(f"  point vs q0.5 max|diff| = {np.max(np.abs(m - q[..., 4])):.2e} (the point forecast is the median); point vs quantile-average = {np.max(np.abs(m - q.mean(-1))):.2e}")
            _checked[0] = True
...
def stage_ladder(args, model, done):
    ...
    for rung in ["N1", "N2", "N3_nu3", "N3_nu5", "N4"]:
        for seed in range(args.seeds):
            ...
            y_hat, q = forecast_tirex(model, y_ctx, H, args.batch); fl = norm_floor(y_ctx, H)
...
def stage_direction(args, model, done):
    ...
    for rung in ("N1", "N2", "N4"):
        ...
        y_hat, _ = forecast_tirex(model, ctx, H, args.batch); dep = y_hat - ctx[:, -1:]
...
def main():
    ...
    args = ap.parse_args()
    log(f"loading {MODEL_ID}"); model = load_tirex(); log(f"{type(model).__name__} loaded")
